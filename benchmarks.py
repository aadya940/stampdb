...
from stampdb.relational import Summation
...
def benchmark_stampdb(times, values):
    ...
    def aggregate():
        ...
        return Summation("temp", data).do() / len(data)
...
def run_benchmarks():
    ...
    print(
        f"TinyFlux: Append={tf_write:.3f}s, Query={tf_query:.3f}s, Aggregate={tf_agg:.3f}s"
    )

    print("Benchmarking StampDB...")
    stamp_write, stamp_query, stamp_agg = benchmark_stampdb(times, values)
    print(
        f"StampDB: Append={stamp_write:.3f}s, Query={stamp_query:.3f}s, Aggregate={stamp_agg:.3f}s"
    )
...
if __name__ == "__main__":
    ...
    for i in range(100):
        tf_write, tf_query, tf_agg, stamp_write, stamp_query, stamp_agg = (
            run_benchmarks()
        )
